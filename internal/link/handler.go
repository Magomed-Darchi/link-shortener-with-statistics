...
func NewLinkHandler(router *http.ServeMux, deps LinkHandlerDeps) {
	...
	router.Handle("PATCH /link/{id}", midlleware.IsAuthed(handler.Update(), deps.Config))
	router.HandleFunc("DELETE /link/{id}", handler.Delete())
	router.HandleFunc("GET /{hash}", handler.GoTo())
	router.Handle("GET/link/{id}", midlleware.IsAuthed(handler.GetAll(), deps.Config))
...
func (handler *LinkHandler) Update() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		email, ok := r.Context().Value(midlleware.ContextEmailKey).(string)
		if ok {
			fmt.Println(email)
		}
